...
public class Evaluator {
    ...
    public Evaluator(DataSet dataSet, LabelType labelType) {
...
// calculating TP,TN,FP,FN
        dataSet.records.stream().forEach(record -> {
    ...
    private void finalMeasurement() {
        ...
        accuracy = (double) Arrays.stream(truePositiveArrayCount).reduce((x, y) -> x + y).getAsInt() / (double) dataSet.records.size();
        errorRate = 1 - accuracy;
        if (labelType.equals(LabelType.fiveStar)) {
            MAE = dataSet.records.stream().map(x -> Math.abs(x.labeledScore - x.estimatedScore)).mapToDouble(Double::valueOf).average().getAsDouble();
        }
